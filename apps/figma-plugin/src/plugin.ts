...
figma.showUI(__html__, { width: 340, height: 530, title: "MenuCast" });
...
figma.ui.onmessage = async (msg: { type: string; payload?: unknown }) => {
  ...
  if (msg.type === "export-and-push") {
    ...
    // Get the current page selection or find first frame
    ...
    try {
      // Export the frame as PNG (2x for crisp TV display)
      ...
      figma.ui.postMessage({
        ...
        payload: {
          ...
          fileName: `${node.name.replace(/[^a-zA-Z0-9_-]/g, "_").toLowerCase()}.png`,
          tvId,
          token,
          appUrl,
        },
